...
class ProgressSweepBookmarkTest {
    ...
    private class FakeResolver(private val available: Set<String>) : SyncSourceResolver {
        override suspend fun resolve(sourceId: String): SyncSource? =
            if (sourceId in available) DummySource else null
    }

    private object DummySource : SyncSource {
    ...
    fun `skips a bookmark item the live player is currently driving consistent with the audio pass`() = runTest {
        ...
        sweep(
            ...
            rec, FakeResolver(setOf("s1")), openTargets,
        ).run()
