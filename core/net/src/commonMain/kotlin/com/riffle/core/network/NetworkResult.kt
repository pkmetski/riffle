...
object KtorClassifier {
    ...
    suspend fun <T> classify(block: suspend () -> T): NetworkResult<T> = try {
        ...
        when {
            isSSLHandshakeException(e) -> NetworkResult.InsecureConnection(InsecureConnectionType.SELF_SIGNED)
            isIOException(e) -> NetworkResult.Offline(e)
            else -> NetworkResult.Unknown(e)
        }
    }
}
...
fun NetworkResult<*>.errorAsThrowable(): Throwable = when (this) {
    ...
    is NetworkResult.ServerError -> newIOException("HTTP $code${errorMessage?.let { ": $it" } ?: ""}")
    NetworkResult.Auth -> newIOException("HTTP 401")
    is NetworkResult.InsecureConnection -> newSSLHandshakeException("Insecure connection ($type)")
}
