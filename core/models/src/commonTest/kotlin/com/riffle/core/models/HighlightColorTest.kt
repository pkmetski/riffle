...
class HighlightColorTest {
    ...
    fun `tokens are the lowercase enum names`() {
        assertEquals(HighlightColor.YELLOW.token, "yellow")
        assertEquals(HighlightColor.GREEN.token, "green")
        assertEquals(HighlightColor.BLUE.token, "blue")
        assertEquals(HighlightColor.RED.token, "red")
    }
