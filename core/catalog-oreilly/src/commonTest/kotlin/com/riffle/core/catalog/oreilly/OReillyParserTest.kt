...
class OReillyParserTest {
    ...
    fun `search parses only books into the books root`() {
        ...
        assertEquals(book.id, "9781617299605")
        assertEquals(book.title, "Kotlin in Action, Second Edition")
        assertEquals(book.author, "Svetlana Isakova, Sebastian Aigner")
        assertEquals(BookFormat.Epub, book.ebookFormat)
        assertEquals(false, book.hasAudio)
        assertEquals(book.publishedYear, "2024")
        assertEquals(book.publisher, "Manning Publications")
        assertEquals(book.language, "en")
        ...
        assertEquals(book.coverUrl, "https://learning.oreilly.com/library/cover/9781617299605/600/")
    }
    ...
    fun `search parses only audiobooks into the audiobooks root`() {
        ...
        assertEquals(audio.id, "9781617299605AU")
    ...
    fun `unknown keys are ignored and hits without an id are dropped`() {
        ...
        assertEquals(1, items.size)
        assertEquals(items.single().id, "x1")
    }
    ...
    fun `book detail maps to a catalog item`() {
        ...
        assertEquals(item.title, "Automate the Boring Stuff with Python, 2nd Edition")
        assertEquals(item.author, "Al Sweigart")
        assertEquals(item.coverUrl, "https://learning.oreilly.com/library/cover/9781098122584/600/")
        assertEquals(item.description, "<span>desc</span>")
        assertEquals(item.language, "en")
        assertEquals(item.isbn, "9781593279929")
        assertEquals(592, item.pageCount) // prefers page_count over virtual_pages
        assertEquals(item.publishedYear, "2019")
    ...
    fun `spine reference_id yields the packaged file path`() {
        ...
        assertEquals(spine.results.first().title, "Cover Page")
    }
    ...
    fun `files parse kinds and paths`() {
        ...
        assertEquals(files.results.last().kind, "stylesheet")
        assertEquals(files.results.last().mediaType, "text/css")
    }
