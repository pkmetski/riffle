...
class PanelDetectorRealImageTest {
    ...
    fun `dark-gutter 3x2 grid from user attachment is detected as multiple panels`() {
        val (grid, w, h) = loadFixture("comic-panel-fixtures/dark_gutter_3x2.png")
        val result = detector.detect(grid, pageIndex = 0, originalWidth = w, originalHeight = h)
        val trace = detector.detectDebug(grid)
        dumpTrace("dark_gutter_3x2", grid, trace)

        println("dark_gutter_3x2 → source=${result.source} panels=${result.panels.size}")
        result.panels.forEachIndexed { i, p ->
            println("  final panel[$i] = (${p.x},${p.y})..(${p.right},${p.bottom})")
        }
        println("  binaryMask=${trace.binaryMaskWidth}x${trace.binaryMaskHeight} contentPixels=${trace.binaryMaskData?.count { it == 1.toByte() }}")
        println("  cropped=${trace.croppedWidth}x${trace.croppedHeight} offset=(${trace.croppedOffsetX},${trace.croppedOffsetY})")
        println("  components(raw)=${trace.componentBboxes.size} filtered=${trace.filteredBboxes.size}")
        trace.filteredBboxes.forEachIndexed { i, b ->
            println("  filtered[$i] = (${b[0]},${b[1]})..(${b[2]},${b[3]})")
        }
        assertNotEquals(
            "expected auto-detection to find multiple panels, got Fallback — see build/panel-detector-debug/dark_gutter_3x2_*.png",
            PanelSource.Fallback,
            result.source,
        )
        assertTrue(
            "expected multiple panels for a 3x2 grid, got ${result.panels.size} — see build/panel-detector-debug/dark_gutter_3x2_*.png",
            result.panels.size >= 4,
        )
    }
    ...
    private fun loadFixture(resourcePath: String): LoadedGrid {
        ...
        // page bytes, before the reader lays them out inside the viewport).
        val source = trimBlackLetterbox(rawSource)
