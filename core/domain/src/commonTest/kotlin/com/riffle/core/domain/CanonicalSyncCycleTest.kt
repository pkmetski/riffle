...
class CanonicalSyncCycleTest {
    ...
    fun `a write the server stamps later than the winner is adopted as the canonical lastUpdate`() = runTest {
        ...
        assertNull(result.jumpTo, "local won, so no jump")
        assertEquals(pos("L"), ebook.patchedWith)
        assertEquals(5_000L, result.canonicalLastUpdate, "the server write timestamp is adopted")
    }
    ...
    fun `zero-peer cycle no-ops on both directions and preserves local lastUpdate`() = runTest {
        ...
        // untouched so on-device activity keeps advancing it (ADR 0049 zero-peer contract).
        val local = LocalCanonical(pos("L"), lastUpdate = 4242)

        val result = CanonicalSyncCycle.run(local, emptyList())

        assertNull(result.jumpTo, "no remotes → no jump")
        assertTrue(result.patched.isEmpty(), "no remotes → nothing patched")
        assertEquals(
            4242L,
    ...
    fun `a peer that skips its PATCH is treated identically to one that fails`() = runTest {
        ...
        assertEquals(setOf("STORYTELLER"), result.patched)
        assertEquals(200L, result.canonicalLastUpdate, "the skipped peer's (non-existent) stamp must not be adopted")
    }
}
