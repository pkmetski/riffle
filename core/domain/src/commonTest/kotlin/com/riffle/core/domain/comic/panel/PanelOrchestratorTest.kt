...
class PanelOrchestratorTest {
    ...
    fun `on a cache miss the detector runs and result is cached`() {
        ...
        assertTrue(
            ...
            "expected first panel near (20,20), got (${page.panels[0].x},${page.panels[0].y})",
        )
        assertTrue(
            page.panels[1].x in 205..215 && page.panels[1].y in 15..25,
            "expected second panel near (210,20), got (${page.panels[1].x},${page.panels[1].y})",
        )
    }
    ...
    fun `resolvePage is idempotent - second call hits the store`() {
        ...
        assertEquals(1, decoder.callCount, "decoder must be called only once")
    }
