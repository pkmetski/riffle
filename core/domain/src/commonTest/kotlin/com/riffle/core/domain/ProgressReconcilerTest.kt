...
class ProgressReconcilerTest {
    ...
    fun `server newer wins and is persisted locally with the server stamp`() = runTest {
        ...
        assertEquals(store.position, "server-cfi")
    ...
    fun `local newer is pushed and adopts the server-returned stamp`() = runTest {
        ...
        assertEquals(remote.patchedWith, "local-cfi")
        assertEquals(store.position, "local-cfi")
    ...
    fun `clean row with local stamp ABOVE server stamp does NOT push (clock-skew guard)`() = runTest {
        ...
        assertEquals(ReconcileOutcome.InSync, outcome)
        assertEquals(store.position, "stale-local-cfi")
    ...
    fun `clean row pulls when server has advanced since last sync (cross-device push)`() = runTest {
        ...
        assertEquals(store.position, "new-server-cfi")
    ...
    fun `clean row with missing server file (lastUpdate=0) re-pushes to restore it`() = runTest {
        ...
        assertEquals(remote.patchedWith, "saved-cfi")
    ...
    fun `ServerWon with deleted=true marks local row deleted and skips uiSink`() = runTest {
        ...
        assertTrue(store.deleted, "row must be marked deleted")
        assertEquals(0, sinkCalls, "uiSink must not be called for a deleted item")
    }
    ...
    fun `ServerWon with deleted=false resets local deleted flag and calls uiSink`() = runTest {
        ...
        assertFalse(store.deleted, "deleted flag must be reset by server win")
        assertEquals(1, sinkCalls, "uiSink must be called for non-deleted server win")
    }
