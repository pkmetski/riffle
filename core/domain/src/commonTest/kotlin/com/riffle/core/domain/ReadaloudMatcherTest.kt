...
class ReadaloudMatcherTest {
    ...
    fun `fuzzy title above threshold with matching author surfaces a Pending candidate`() {
        ...
        assertEquals(candidate.absServerUuid, "abs-1")
        assertEquals(candidate.absLibraryItemId, "item-1")
        assertTrue(candidate.score >= ReadaloudMatcher.FUZZY_THRESHOLD, "score ${candidate.score} should be >= threshold")
    }
