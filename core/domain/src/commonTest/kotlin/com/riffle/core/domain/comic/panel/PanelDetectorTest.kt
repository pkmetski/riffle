...
class PanelDetectorTest {
    ...
    fun `duplicate panels are deduped so the reader doesn't show the same panel twice`() {
        ...
        assertEquals(PanelSource.Auto, result.source)
        assertEquals(2, result.panels.size, "expected 2 clean panels, got ${result.panels}")
    ...
    fun `bbox that straddles two real panels plus a gutter is split at the gutter`() {
        ...
        assertEquals(
            2,
            result.panels.size,
            "expected 2 panels after split-at-internal-gutter, got ${result.panels.map { "(${it.x},${it.y})${it.width}x${it.height}" }}",
        )
        // Panels should each be ~160 wide, not one ~360-wide bbox spanning the gutter.
        for (p in result.panels) {
            assertTrue(p.width < 250, "panel width ${p.width} should be < 250 (i.e. not spanning both panels + gutter)")
        }
    }
    ...
    fun `wide hollow-bordered panels are not falsely split at low-content interior rows`() {
        ...
        assertEquals(
            2,
            result.panels.size,
            "expected 2 panels (wide hollow rects), got ${result.panels.map { "(${it.x},${it.y})${it.width}x${it.height}" }}",
        )
    }
    ...
    fun `multi-row page with sparse-interior panels detects all rows`() {
        ...
        assertEquals(
            8,
            result.panels.size,
            "expected 8 panels (4-row grid of hollow rects), got ${result.panels.map { "(${it.x},${it.y})${it.width}x${it.height}" }}",
        )
    }
    ...
    fun `full-width splash row above two side-by-side panels yields 3 regions`() {
        ...
        assertEquals(1, topPanels.size, "expected 1 top splash panel")
        assertEquals(2, bottomPanels.size, "expected 2 bottom panels")
        assertTrue(
    ...
    fun `two full-width rows above two side-by-side panels yield 4 regions`() {
        ...
        assertEquals(2, fullWidthRows.size, "expected 2 full-width panels")
        assertEquals(2, bottomPanels.size, "expected 2 narrow bottom panels")
    }
    ...
    fun `top strip with dark-bordered gutters is split into 4 panels`() {
        ...
        assertEquals(
            5,
            result.panels.size,
            "expected 5 panels (4 top strip + 1 large), got ${result.panels.map { "(${it.x},${it.y})${it.width}x${it.height}" }}",
        )
        val topPanels = result.panels.filter { it.y < 170 }
        assertEquals(4, topPanels.size, "expected 4 top-strip panels")
    }
    ...
    fun `panel with flood-fill-accessible interior is not falsely split by projection path`() {
        ...
        assertEquals(
            4,
            result.panels.size,
            "expected 4 panels, got ${result.panels.map { "(${it.x},${it.y})${it.width}x${it.height}" }}",
        )
        val topLeft = result.panels.filter { it.x < 240 && it.y < 200 }
        assertEquals(1, topLeft.size, "top-left panel with flood-fill-accessible interior must survive as one panel")
        assertTrue(
            topLeft[0].height >= 150,
            "top-left panel must span the full panel height, not just a falsely-split remnant",
        )
    }
    ...
    fun `2x2 grid with narrow horizontal gutter is split correctly via flood-fill`() {
        ...
        val topPanels = result.panels.filter { it.y < 200 }
        val bottomPanels = result.panels.filter { it.y >= 200 }
        assertEquals(2, topPanels.size, "expected 2 top panels")
        assertEquals(2, bottomPanels.size, "expected 2 bottom panels")
    }
    ...
    fun `mergeDiagonalSpanningPanels joins a column panel with the full-width band below it`() {
        ...
        assertEquals(3, merged.size, "expected tall-left + row1-right + row2-right; got $merged")
        assertTrue(
    ...
    fun `mergeDiagonalSpanningPanels full-width band threshold boundary`() {
        ...
        assertEquals(setOf(top, botBelow), refused.toSet(), "84%-wide bot must not merge; got $refused")

        val merged = detector.mergeDiagonalSpanningPanels(listOf(top, botAbove), pageW, pageH)
        assertEquals(2, merged.size, "86%-wide bot must merge into tall + remainder; got $merged")
        assertTrue(
    ...
    fun `merged remainder follows the diagonal gutter into the band`() {
        ...
        assertEquals(2, merged.size, "expected tall + remainder; got $merged")
        assertTrue(
    ...
    fun `merged remainder left edge is walked back to keep overlap under the sanity cap`() {
        ...
        assertEquals(431, remainder.minX, "remainder left edge must be capped at 431; got $remainder")
    ...
    fun `merge result is independent of input bbox order`() {
        ...
        assertEquals(forward.toSet(), reversed.toSet(), "reversed input must merge identically; got $reversed vs $forward")
    ...
    fun `expandDiagonalBboxOverlaps caps the pad so overlap stays under the sanity threshold`() {
        ...
        assertTrue(cLeft.maxX > 499 && cRight.minX < 442, "pad must still expand the pair (cap ≠ no-op); got $capped")
    ...
    fun `mergeSharedBorderFalseGaps merges two same-row panels when at least two panels below straddle the gap`() {
        ...
        assertEquals(3, result.size, "expected 3 panels after merge (merged top-row pair + rows 2-3); got=$result")
        val topPanel = result.firstOrNull { it.y + it.height / 2 < 500 }
        assertNotNull(topPanel, "expected one merged top-row panel; got=$result")
        assertEquals(100, topPanel!!.x, "merged top panel must span from x=100; got=$result")
        assertEquals(1400, topPanel.width, "merged top panel width must be 1400; got=$result")
    }
    ...
    fun `mergeSharedBorderFalseGaps does NOT merge when only one panel below spans the full gap`() {
        ...
        assertEquals(3, result.size, "panels must NOT be merged when < 2 panels span the gap below; got=$result")
    }
    ...
    fun `mergeSharedBorderFalseGaps does NOT merge non-topmost pair even when two rows below span the gap`() {
        ...
        assertEquals(5, result.size, "must NOT merge a non-topmost pair even with 2 straddling rows below; got=$result")
    }
    ...
    fun `mergeSharedBorderFalseGaps does NOT merge when adjacent row has a column boundary at the gap`() {
        ...
        assertEquals(6, result.size, "must NOT merge when adjacent row has a column boundary near gap_left; got=$result")
    }
    ...
    fun `mergeSharedBorderFalseGaps does NOT merge when gap exceeds 7pct of page width`() {
        ...
        val panels = listOf(p0, p1, p2)
        val result = detector.mergeSharedBorderFalseGaps(panels, pageW)
        assertEquals(3, result.size, "panels must NOT be merged when gap > 7% of page width; got=$result")
    }
    ...
    fun `repairMisalignedStackedRowBoundaries realigns a false top-row split to the bottom row boundary`() {
        ...
        assertEquals(3, result.size, "expected 3 panels after realignment; got=$result")
        val spanning = result.firstOrNull { it.minX <= 10 && it.maxY >= 990 && it.minY <= 100 }
        assertNotNull(spanning, "expected a spanning left column (minX ≤ 10, y 100..990); got=$result")
        assertTrue(spanning!!.maxX <= 800, "spanning column must end left of the real boundary; got=$spanning")
        val trimmedTopRight = result.firstOrNull { it.minY <= 100 && it.maxY <= 519 && it.minX >= 830 }
        assertNotNull(trimmedTopRight, "expected top-right cell re-trimmed to start at the real boundary; got=$result")
    }
    ...
    fun `repairMisalignedStackedRowBoundaries does NOT fire when nothing bridges the row gap`() {
        ...
        val input = listOf(stackedTopLeft, stackedTopRight, stackedBottomLeft, stackedBottomRight)
        val result = detector.repairMisalignedStackedRowBoundaries(input, cropped, 1000)
        assertEquals(input.toSet(), result.toSet(), "staggered rows must not be realigned; got=$result")
    }
    ...
    fun `repairMisalignedStackedRowBoundaries does NOT fire when both boundaries are blocked through the other row`() {
        ...
        val input = listOf(stackedTopLeft, stackedTopRight, stackedBottomLeft, stackedBottomRight)
        val result = detector.repairMisalignedStackedRowBoundaries(input, cropped, 1000)
        assertEquals(input.toSet(), result.toSet(), "ambiguous blocking must not realign; got=$result")
    }
    ...
    fun `repairMisalignedStackedRowBoundaries does NOT fire when boundaries are aligned`() {
        ...
        val result = detector.repairMisalignedStackedRowBoundaries(input, cropped, 1000)
        assertEquals(input.toSet(), result.toSet(), "aligned 2x2 grid must not be realigned; got=$result")
    }
    ...
    fun `mergeCrossContainedBboxes merges a tall pillar with a wider row cell it cross-contains`() {
        ...
        assertEquals(2, result.size, "cross-contained pair must merge; got=$result")
        assertTrue(
    ...
    fun `mergeCrossContainedBboxes does NOT merge disjoint or fully nested bboxes`() {
        ...
        assertEquals(4, result.size, "disjoint and fully-nested bboxes must not merge; got=$result")
    }
