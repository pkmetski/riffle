...
class PanelOrdererTest {
    ...
    fun `split middle column does not exile the bottom-half continuation panel`() {
        ...
        assertEquals(
            listOf(20, 280, 280, 540),
            ordered.map { it.x },
            "all four panels should be in one row sorted left-to-right",
        )
        assertEquals(
            listOf(40, 40, 230, 40),
            ordered.map { it.y },
            "within same x, top panel (B) precedes continuation (C)",
        )
    }
    ...
    fun `leftward-descending terrace reads left-to-right as a single band`() {
        ...
        assertEquals(
            listOf(0, 200, 400),
            ordered.map { it.x },
            "terrace panels form one row, read left-to-right",
        )
    }
    ...
    fun `caption strip does not collapse the band and exile the panel below it`() {
        ...
        assertEquals(
            listOf(20, 220, 220, 620),
            ordered.map { it.x },
            "all four in one row, left-to-right (d before b at same x by y)",
        )
        assertEquals(
            listOf(20, 20, 80, 20),
            ordered.map { it.y },
            "D (top) precedes B (below it) at the same x column",
        )
    }
    ...
    fun `CE1 result is stable regardless of input emission order`() {
        ...
        for (input in listOf(
            ...
            assertEquals(expectedX, ordered.map { it.x }, "emission order $input → wrong x sequence")
            assertEquals(expectedY, ordered.map { it.y }, "emission order $input → wrong y sequence")
        }
    }
    ...
    fun `two panels sharing identical y are placed in the same row left-to-right issue 896`() {
        ...
        assertEquals(0, ordered[0].x, "left panel (x=0) must come first in reading order")
        assertEquals(449, ordered[1].x, "right panel (x=449) must come second in reading order")
    }
    ...
    fun `panel starting one pixel below a spanning member is still exiled to the next row`() {
        ...
        assertEquals(spanning, ordered[0], "spanning panel must be first")
        assertEquals(lower, ordered[1], "lower panel must be exiled to second row")
    }
    ...
    fun `tall spanning panel still exiles the lower panel in its own column issue 780 shape`() {
        ...
        assertEquals(tallLeft, ordered[0], "tall-left must be first")
        assertEquals(upperRight, ordered[1], "upper-right must be second")
        assertEquals(lowerRight, ordered[2], "lower-right must be last")
    }
