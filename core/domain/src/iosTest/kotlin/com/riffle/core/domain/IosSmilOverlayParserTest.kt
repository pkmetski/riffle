...
/**
 * iOS counterpart to core:domain's jvmTest SmilOverlayParserTest — the same four scenarios against
 * the same fixtures, driving the ksoup-based [IosSmilOverlayParser] instead of the JVM DOM one.
 * The two parsers must agree clip-for-clip: a divergence desynchronises the readaloud highlight.
 */
class IosSmilOverlayParserTest {
    ...
    fun `parses a single par into one clip`() {
        ...
        val clips = IosSmilOverlayParser.parse(smil)
    ...
    fun `preserves document order across multiple pars in a seq`() {
        ...
        val refs = IosSmilOverlayParser.parse(smil).map { it.textFragmentRef }
    ...
    fun `returns empty list for a SMIL with no pars`() {
        ...
        assertEquals(emptyList(), IosSmilOverlayParser.parse(smil))
    }
}
