...
interface SourceDao {
    ...
    // iOS driver runs with foreign-key enforcement off, so the rows outlived the source and
    // leaked one per add/remove cycle with nothing to ever collect them. (iOS does not read the
    // persisted scale yet — LibraryItemsScreen passes a fixed 1f — so the stale value is not
    // user-visible today, but it would be inherited the moment pinch-zoom is wired there.)
    // Delete it explicitly like every other table here, so the graph does not depend on who
