 ...
 * already backfilled `library_items.sourceId`, which it has by the time any DAO query returns.
 */
    ...
    suspend fun migrate() = withContext(dispatchers.io) {
        for ((dir, extension) in stores) {
            ...
            for (file in flatFiles) {
                ...
                val sourceId = resolveServerId(itemId)
                if (sourceId == null) {
                    // Orphan: no library item owns this id anymore.
                    file.delete()
                    continue
                }
                val destDir = dir.resolve(sourceId).also { it.mkdirs() }
