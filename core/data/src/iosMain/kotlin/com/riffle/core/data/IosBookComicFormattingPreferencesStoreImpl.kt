...
// Mirrors BookComicFormattingPreferencesStoreImpl (Android). bookId is "<sourceId>::<itemId>".
internal class IosBookComicFormattingPreferencesStoreImpl(
...
private fun BookComicFormattingPreferencesEntity.toDomain() = BookComicFormattingOverrides(
    backgroundTheme = backgroundTheme?.let { runCatching { ReaderTheme.valueOf(it) }.getOrNull() },
    panelViewOn = panelViewOn,
    panelOverflow = panelOverflow?.let { runCatching { PanelOverflowBehavior.valueOf(it) }.getOrNull() },
