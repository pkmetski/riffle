 ...
 * layer (via [resolveAgainstBase]); by the time a URL reaches this client it's
 * already ASCII-safe.
 */
class ChitankaHttpClient(
    ...
    companion object {
        ...
        internal fun parseXingFrameCount(bytes: ByteArray, frameStart: Int, meta: Mp3FrameMeta): Int? {
            ...
            // decodeToString over String(bytes, off, len, US_ASCII): the Charset-taking String
            // constructor is JVM-only. Both map the ASCII bytes "Xing"/"Info" identically, and
            // any non-ASCII byte fails the equality check either way.
            val tag = bytes.decodeToString(tagOffset, tagOffset + 4)
