 ...
 * separate metadata fields with `<br>`, and ksoup's `.text()` collapses them into a single
...
/**
 * Orders Bulgarian category labels the way `java.text.Collator.getInstance(Locale.of("bg"))`
 * did before the scrapers moved to `commonMain` — there is no `Collator` outside the JVM.
 *
 * The Bulgarian alphabet (А Б В Г Д Е Ж З И Й К Л М Н О П Р С Т У Ф Х Ц Ч Ш Щ Ъ Ь Ю Я) is a
 * subsequence of the Unicode Cyrillic block **in the same relative order**, and the block sits
 * after digits and Latin, matching ICU's root collation. So comparing the lowercased labels by
 * code point reproduces Bulgarian collation for every label chitanka.info emits, with a
 * case-sensitive tie-break purely so the ordering is total and deterministic.
 */
internal val BULGARIAN_LABEL_ORDER: Comparator<String> = Comparator { a, b ->
    val byLetter = a.lowercase().compareTo(b.lowercase())
    if (byLetter != 0) byLetter else a.compareTo(b)
}

/**
 * Kotlin/ksoup port of `lib/scraper/chitanka.ts` from the reference
...
internal object ChitankaScraper {
     ...
     * [resolveAgainstBase] rejects with `IllegalArgumentException` — encode them to `%20` up
     * front so the parse succeeds and the resulting URL is actually usable by the HTTP client.
     * Without this, HEAD-based per-track duration probing collapses to 0 for any book whose
     * filenames contain spaces and the chapter drawer renders every entry as `0:00 / 0m`.
     */
    ...
    fun parseDetailPage(html: String, pageUrl: String): ChitankaDetail {
        ...
        val popoverDesc = if (popoverHtml.isNotEmpty() && !popoverHtml.contains("blockquote")) {
            Ksoup.parse(popoverHtml).selectFirst("p")?.text()?.trim().orEmpty()
        } else ""
    ...
    fun parseCategories(html: String): List<ChitankaCategoryEntry> {
        ...
        return entries.sortedWith(compareBy(BULGARIAN_LABEL_ORDER) { it.label })
    }
...
/**
 * Kotlin/ksoup port of `lib/scraper/gramofonche.ts`. Same 1:1 fidelity as [ChitankaScraper].
 */
