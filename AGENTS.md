...
The multi-platform-core modules (`core:common`, `core:models`, `core:domain`, `core:network`, `core:sources`, `core:sync`, `core:annotations`) must stay pure-Kotlin so a future KMP target can consume them unchanged. The `checkNoAndroidImports` gradle task (wired into `check`) fails CI if any Kotlin file in those modules imports `android.*`, `androidx.*` (except `androidx.annotation`), or `java.util.logging`. `core:annotations` does not exist yet — the check no-ops for missing directories and activates automatically when the module is created. If a file legitimately needs an Android dependency it belongs in an Android-hosting module (`core:data`, `core:database`, `core:database-api`, `core:logging`, `app`) not the pure-Kotlin core; only in exceptional cases add the file's path to `AndroidImportLint.ALLOWLIST` in `buildSrc/` with a one-line justification. Detection logic lives in `buildSrc/src/main/kotlin/com/riffle/buildlogic/AndroidImportLint.kt`. See [ADR 0049](docs/adr/0049-platform-agnostic-core-boundary.md) for the full rationale and module map.
