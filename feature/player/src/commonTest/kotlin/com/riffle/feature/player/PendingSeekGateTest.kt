 ...
 * fallback (which, in the real controller, reads the client-side mirror that has optimistically
 * snapped to the raw local-track offsetMs and would be projected as an absurdly early absolute
 * position). If someone reverts the gate, [pendingSampleWinsOverFallback] flips red.
 */
class PendingSeekGateTest {
    ...
    fun pendingSampleWinsOverFallback() {
        ...
        gate.onSeekIssued(absoluteSec = 10_217.0)
