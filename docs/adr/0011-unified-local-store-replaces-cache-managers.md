...
> **Amended by [ADR 0049](0049-source-and-service-abstractions-replace-server.md)** — storage is scoped per **Source** rather than globally. The two-tier Cache/Download split is preserved for Sources with remote catalogs; **[LocalFiles]** has one tier (permanent, copy-in) because its files are always local.
