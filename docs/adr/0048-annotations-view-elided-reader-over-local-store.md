# ADR 0048 — Annotations view is an elided reader over the local annotation store
...
- PDF books are format-neutrally excluded: the list query filters `WHERE type='HIGHLIGHT' AND deleted=false`, and [Annotation]s are anchored on EPUB CFI (ADR 0028). Adding a PDF-highlight anchor variant later is additive — the drawer entry, the list, and the sort key stay unchanged.
