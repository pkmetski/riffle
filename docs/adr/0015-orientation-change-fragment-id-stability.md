...
**Fragment restoration crash on second rotation (fixed via `SafeNavigatorFragmentFactory`)** — After the first rotation the FM saves the newly created `EpubNavigatorFragment` (which has no public no-arg constructor). On the NEXT Activity recreation `FragmentManager.restoreSaveStateInternal()` fires inside `FragmentActivity.onCreate`, before any Compose code can run, and tries to instantiate the fragment via reflection → `Fragment$InstantiationException`. The fix registers `SafeNavigatorFragmentFactory` on `supportFragmentManager` in `MainActivity.onCreate` **before** `super.onCreate()`. The safe factory catches the `InstantiationException` and returns a plain `Fragment()` placeholder; the reader screen's `AndroidView.update` detects the placeholder (via `fragmentRef.value == null`) and removes it before recreating with the proper factory.
