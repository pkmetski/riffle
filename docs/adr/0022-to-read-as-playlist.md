# ADR 0022 — "To Read" implemented as a name-matched ABS Playlist

> **Amended by [ADR 0049](0049-source-and-service-abstractions-replace-server.md)** — To Read is now gated on `PlaylistsCapability`. Sources without the capability (LocalFiles today, OPDS/Calibre/Gutenberg in future) hide the To Read tab entirely. The ABS implementation is unchanged.
...
- **Legacy "To Read" Collections on existing servers are abandoned.** Users with pre-ADR-0022 history have a stranded `To Read` Collection visible in the ABS web UI's Collections tab. The app does not delete it, migrate from it, or read from it. Users may delete it manually.
