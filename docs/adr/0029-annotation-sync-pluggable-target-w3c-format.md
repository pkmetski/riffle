# ADR 0029 — Annotation sync: pluggable target, W3C Web Annotation format, per-device-file merge
...
**Superseded-in-part by:** [ADR 0045](0045-annotation-tombstone-ttl-and-file-gc.md) — replaces the "No automatic GC" policy and the "Manual tombstone compaction was prototyped and removed" discussion with a wall-clock TTL sweep of tombstones plus WebDAV file deletion when a device's file becomes empty.
