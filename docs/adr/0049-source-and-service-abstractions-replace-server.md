...
- **The `library_items` table graduates.** Today it doubles as ABS-catalog mirror and reader's library, and its identity — item id alone — collides across Servers ([ADR 0031]). Under this ADR the mirror becomes a per-Source `catalog_cache` (opt-in via `OfflineBrowseCapability`), and every table that references an item keys on `(sourceId, sourceItemId)`. This ships the fix that [ADR 0031] tabled as future work.
...
- **Storyteller loses its "Server" clothing.** It has no library, no browse, no Source Switcher presence — it's a Service. The layered justifications in [ADR 0024] and [ADR 0032] collapse into "Storyteller is a Service that provides `ReadaloudProvider`."
...
- **[ADR 0022]** (To Read as Playlist): gated on `PlaylistsCapability`; hidden for Sources without.
- **[ADR 0024]** (Storyteller as peer server): superseded — Storyteller is a Service.
- **[ADR 0031]** (Key local stores by Server and Item): superseded — shipping design, renamed to Source.
- **[ADR 0032]** (Storyteller as Settings-only backend): superseded — Storyteller is a Service (Settings-only follows from the category).
- **[ADR 0033]** (Client-side facet filtering): amended — over the active Source's catalog cache when present, or over acquired items otherwise.
- **[ADR 0035]** (Audiobook direct-ABS streaming): amended — any Source implementing `AudiobookMediaCapability`; ABS is the reference implementation.
