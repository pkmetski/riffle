# ADR 0050 — Comic Book Archive (CBZ) support
...
- Features that are text-domain — **[Book Search]**, **[Annotation]s** (Highlight/Note/Bookmark), **[Readaloud]**, **[Cadence]** — remain unavailable for comics, following the same pattern that already excludes PDF from Annotations and Book Search. Comics never appear in the [Annotations View] for the same reason PDFs don't ([ADR 0028]).
...
- **[ADR 0028]** (Annotations anchor on EPUB CFI): unchanged. Comics carry no annotations in v1 for the same reason PDFs don't.
