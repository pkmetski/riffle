# ADR 0031 — Key Library Items by (serverId, itemId) end-to-end

> **Superseded by [ADR 0049](0049-source-and-service-abstractions-replace-server.md)** — the same design (composite key), renamed: `serverId` → `sourceId`. The Source/Service re-root ships this decision that ADR 0031 had left as future work. Design A holds unchanged.
