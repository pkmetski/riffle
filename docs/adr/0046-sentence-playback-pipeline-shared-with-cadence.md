# ADR 0046 — Sentence-highlight pipeline generalized for Cadence alongside Readaloud
...
Cadence's chapter navigation is text-domain — the ticker's `next` past the last fragment of a chapter calls the shared controller's `goForward`, which auto-advances at the boundary (see ADR 0047).
