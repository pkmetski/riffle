# ADR 0024 — Storyteller as a peer Server type; Readaloud Library as its single navigation surface

> **Further superseded by [ADR 0049](0049-source-and-service-abstractions-replace-server.md)** — Storyteller is a **Service** (not a Server or Source of any kind). The "peer Server" framing this ADR walked back is now moot: the taxonomy has no such category. Kept for history.

**Status:** Superseded by [ADR 0032](0032-storyteller-as-settings-only-readaloud-backend.md)

> **Superseded.** Storyteller is no longer a browsable peer Server and contributes no Readaloud Library. It is a Settings-only Readaloud backend; the Readaloud capability surfaces only on matched ABS Library Items, and the ABS→Storyteller metadata-borrow rule is dropped. Storyteller-only deployments are no longer supported. The matching mechanism ([ADR 0025](0025-storyteller-abs-matching-with-review-queue.md)) and three-peer sync ([ADR 0023](0023-three-peer-unified-canonical-progress-sync.md)) survive. The historical record below is retained for context only.
...
The matching link's purpose is **cross-Server progress sync** (per [ADR 0023](0023-three-peer-unified-canonical-progress-sync.md)) and **display-metadata sourcing**, not the collapse of entry points.
...
- This ADR does not specify the matching algorithm or its review UI — see [ADR 0025](0025-storyteller-abs-matching-with-review-queue.md).
...
- **Case 2 — ABS-side, Confirmed match.** Same as Case 1, plus a footer line below the action row: "Readaloud available — open from *[Readaloud Library name]*" that tap-navigates to the matched Library Item on the Readaloud side. Reading progress shown is the unified canonical position from [ADR 0023](0023-three-peer-unified-canonical-progress-sync.md) — reading from the ABS side participates in three-peer sync once prerequisites are cached. **Library card badge:** a small readaloud glyph also appears on the ABS-side Library card in the grid, signalling availability before the user opens the Detail Screen.
- **Case 3 — Readaloud-side, no match (Storyteller-only or Unmatched).** Display metadata is Storyteller-native — limited to cover, title, and authors (the API surfaces nothing else). Read button opens the Storyteller EPUB with the headphones icon ready in the TopAppBar. Two independent download actions: **Download** (EPUB bundle) and **Download readaloud audio** (audiobook bundle). Footer: "Not linked to an ABS book — *Pair manually*" (the unmatched-pairing entry point from [ADR 0025](0025-storyteller-abs-matching-with-review-queue.md)). When neither bundle is present and the device is offline, the Read button is disabled with a "Connect to download book" tooltip.
- **Case 4 — Readaloud-side, Confirmed match.** Same as Case 3, with display metadata sourced from the linked ABS item, and the footer changed to "Linked to: *[ABS title] · [Library]*" with an unlink icon. Reading progress shown is the unified canonical position from [ADR 0023](0023-three-peer-unified-canonical-progress-sync.md).
