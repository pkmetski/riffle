...
Modules marked **[guarded]** are actively scanned by `checkNoAndroidImports` on every CI push. Unguarded catalog modules are pure-Kotlin by convention but not yet in the scan set (see ADR 0059).
...
| `core:database-api` | Room `@Entity` / `@Dao` interfaces (Android-only; Room KMP pending — ADR 0058) |
