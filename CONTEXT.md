...
### Source
A user-configured browsable origin of items with a library view. Types shipping today: **ABS** (Audiobookshelf) and **LocalFiles** (one or more device folders). Types the abstraction plans to accept without a shipping implementation: **OPDS**, **Calibre**, **Komga**, **Kavita**, **Gutenberg**. Every Source implements the [Catalog] interface — a small mandatory core plus opt-in [Catalog Capability] mixins that determine which features surface for that Source. Multiple Sources can be configured; the user switches between them via the [Source Switcher]. **Downloads, reading progress, and annotations are scoped per Source** — the same book on two Sources is deliberately two separate entries with two separate reading experiences (rationale: resilience via re-add; see [ADR 0049]). Configured in Settings; listed in the [Source Switcher] and [Navigation Drawer].

### Server Source
A Source backed by a user's configured server and its persistent library. The user's server-side catalogue and account state are the source of truth, and the Source may expose server capabilities such as progress synchronization or accepting library mutations. Importing a book and synchronizing progress are independent capabilities: a Server Source may accept an upload even when it cannot store or synchronize reading progress. Shipping Server Sources are **ABS** and **Komga**. A Server Source is distinct from a [Web Source]: a server may be reachable over the web, but it is not merely a public web catalogue.

### Web Source
A Source backed by a remotely browsable web catalogue rather than the user's persistent server library. Web Sources expose catalogue-owned works for browsing and file acquisition; they do not represent a user-owned server library. A Web Source may be **anonymous** (Chitanka, Gutenberg — no login) or **credentialed** (**[O'Reilly]** — a third-party service the user authenticates against). The credentialed case still differs from a [Server Source]: the catalogue belongs to a third party the user does not host, the network host is fixed rather than user-entered, and there is no server-side [Progress Sync] peer to reconcile against. Shipping Web Sources are **Chitanka**, **Gutenberg**, and **[O'Reilly]** ([Developer Options]–gated). A Web Source is distinct from a [Server Source], even though both may use HTTP and both may require credentials. [LocalFiles] is neither: it is the device's own file origin.

### Service
A sidecar that enriches items regardless of which Source they came from. Distinct from a [Source]: has no library, no browse surface, never appears in the [Source Switcher] or [Navigation Drawer], configured only in Settings. Concrete Services: **[Storyteller Service]** (produces the [Readaloud Sidecar] and audio plan for a matched item) and **sync targets** (WebDAV and local-directory today; Google Drive and native provider annotation APIs are future slots). The WebDAV sync target covers two orthogonal concerns: **annotation sync** (for sources with a `ProgressPeerCapability`, such as Komga) and **reading progress sync** (for [Web Source]s, which have no server-side progress peer — a single `{namespace}__{itemId}__progress.json` file per book carries the position, fraction, and timestamp). See [ADR 0049].
...
### O'Reilly
The Source type backing [learning.oreilly.com](https://learning.oreilly.com). A **credentialed [Web Source]** gated behind [Developer Options]: its card appears in the "Add source" picker only when dev mode is unlocked (add-time gate only — see [Developer Options]). Singleton (one account per device), fixed host, authenticated by **email + password**. Unbounded catalog — browsed remotely, never Room-mirrored — exposing two [Libraries]: **Books** (ebooks) and **Audiobooks**; other content kinds are filtered out at the catalog layer and never surface. From the reader, player, and [Downloads Screen] it behaves like any other network-backed Source: ebooks cache on open (promotable to a permanent [Download]); audiobooks stream via `AudiobookMediaCapability` with chapter markers and are likewise downloadable. Standard library search applies.

Implements the mandatory Catalog core plus `AudiobookMediaCapability` (chaptered). No `SeriesCapability`, `CollectionsCapability`, `PlaylistsCapability` (To Read tab hidden; declared `ToReadSupport.LocalOnly`), `ProgressPeerCapability` (progress is device-local), `ReadingSessionsCapability`, `StatsCapability`, or `OfflineBrowseCapability`. Annotations are `SyncNamespace.LocalOnly`.

### Category Facet
A **server-side** filter chip strip inside a Library screen, driven by facets the [Catalog] enumerates via `listFacets(rootId)`. Distinct from the client-side [Filtered Books Screen] (ADR 0033): client-side facets filter the already-synced-in-memory set; a Category Facet is a hint passed back to `Catalog.browse(rootId, facet=…)` and executes on the origin (e.g. Chitanka fetches `/texts/label/{genre}`). Rendered as a single-select horizontal chip strip below the search bar; "All" chip on the left. Catalogs with no server-side facets return `emptyList()` from `listFacets` and no strip renders. See [ADR 0051].

### Bookshelf
A synthetic first-party navigation entry pinned at the top of the [Navigation Drawer], above the [Source Switcher] and separated from source-specific content by a divider. Not a real [Source] — it has no catalog, no backend, and does not appear in the [Source Switcher] dropdown. Its icon is the Riffle app logo (rounded). Opens a dedicated 3-tab screen (In Progress, To Read, Annotations) that aggregates content across all configured Sources:
- **In Progress tab**: shows In Progress and Continue Series sections, identical in layout to the [Home Tab], with a source badge on each cover indicating which Source the item belongs to.
- **To Read tab**: flat cover grid of all items in any Source's To Read list, with source badge.
- **Annotations tab**: list of all books with highlights across all Sources, reusing the [Annotations View] presentation, with source badge.

The cover grid reuses the same `LazyVerticalGrid` + `coverGridMinCellSize()` + `LocalCoverGridScale` adaptive sizing system as other Library screens; pinch-to-zoom scale is stored in the global DataStore (not per sourceId/libraryId). Tapping an item navigates to its [Library Item Detail Screen] on the owning Source. The Bookshelf screen itself does not support browsing or filtering — it is a reading-activity view, not a catalog surface.

### Navigation Drawer
The primary navigation surface. Contains: a pinned [Bookshelf] entry at the top (with a divider below it), the active Source name in a header (tappable → [Source Switcher] dropdown), the Source-ordered list of visible Libraries for the active Source, a Downloads entry, and a Settings entry — in that order. Replaces the standalone LibraryListScreen and ServerListScreen. On a fresh install with no Sources configured, the app opens directly to Add-Source; after the first Source is added, it navigates to the first Library in that Source's [Library Visibility Preferences] list.
...
A screen displaying the full metadata for a single Library Item: cover, title, author, series membership (shown as `<series name> #<sequence>` when the Source supports Series), description, published year, genres, language, and publisher. Also shows reading progress and local availability via a [Download Button]. EPUB details show a personalized estimated total reading time, based on the global reading speed learned from prior sessions, plus estimated remaining time while the book is in progress. PDF and [Comic Book Archive] details show their discrete page count. These file-derived facts use an inode-aware, TTL-bound local cache rather than permanent Source metadata. For Readable items, contains a Read button that launches the appropriate reader (EPUB or PDF). For [Unsupported Library Item]s, the Read button is absent and the screen explains that no ebook file is available. Reachable by tapping any Library Item card in the library list, a Series detail, or a Collection detail. Back-navigating from the reader returns here before returning to the list.
...
### Download Button
An icon-only button on the [Library Item Detail Screen] that manages the local copy of an item. Cycles through four action states: outline arrow (not downloaded), differently-coloured filled arrow (cached but not downloaded — tap to promote the Cache to a Download), spinner (download in progress), and primary filled arrow (downloaded — tap to remove). Removing a download clears any same-item cached copy too, so the item does not degrade back to cached. Removing is immediate, no Undo, no confirmation dialog; re-downloading is a deliberate user action.

### Cache
An implicit, evictable local copy of a Library Item's content file, created automatically when the user opens a book from a Source with a remote catalog. Stored in a clearable system cache directory. Available for offline reading as long as it has not been evicted by the OS or by Riffle's configurable age-after-access cleanup. Cache cleanup applies only to content artifacts (EPUB, PDF, audiobook track directories, CBZ, and future registered content-file formats); it does not clear library records, metadata, covers, readaloud sidecars, or permanent Downloads. Applies to Sources whose files arrive over the network (**[ABS]** today; future OPDS/Calibre/Komga/Kavita/Gutenberg). **Does not exist for [LocalFiles]** — its files are always local by copy-in, not evictable, and never carry a Cache tier.
...
The Cached section includes a **Cache settings** button. The setting controls age-after-access auto-clear for cached content files only; explicit Downloads are never auto-cleared. There is still no user-facing cache-size cap. A cross-Source aggregate footprint may surface in Settings as a summary; the per-Source screen is the operational surface.

### Offline Mode
The state in which the active Source cannot be reached — either because the device has no network or because the Source's backend is unreachable. Detected reactively. In this state the reader reads from [Cache] or [Download]. Reading and listening progress recorded during Offline Mode is stored locally and **durably reconciled when connectivity returns**, whether or not the book is reopened (see [ADR 0036]) — for Sources implementing `ProgressPeerCapability`. A newer server position is never overwritten. For Sources without `ProgressPeerCapability` (e.g. [LocalFiles]), all progress is local by nature and Offline Mode is not a meaningful state.
...
- **No conflict prompt.**
- **Durable across Offline Mode:** reconciled when connectivity returns (see [ADR 0036]), GET-before-PATCH so a newer server position is never overwritten.
- **Zero-peer case:** for Sources without `ProgressPeerCapability` (e.g. [LocalFiles]), the cycle no-ops; progress is purely local.
- **Matched Readaloud on ABS:** the book has two Peers — ABS ebook progress and ABS audiobook progress — both first-class ([ADR 0035]). Storyteller's own position record is not a Peer; the bundle's SMIL is used only to translate audio ↔ text.
...
- **[Comic Book Archive]:** none in v1. Fit Whole, paginated, LTR are all hard-coded; the reader shows no Formatting Preferences button (see [ADR 0050]).
...
### Annotation
Umbrella term for the user-authored marks attached to a Library Item: [Highlight], [Note], [Bookmark], and [Emphasis]. All share one storage model and one [Annotation Sync] format. Each carries a stable client-generated ID, creation and last-modified timestamps, and a device-origin tag, and anchors to an EPUB CFI. Every Annotation additionally stores the surrounding text snippet and chapter href as a human-readable fallback.

Annotations are keyed by the Library Item's `(sourceId, sourceItemId)`. Since every book is read from the Source's own EPUB, Annotations are available during all reading, with or without a Readaloud. Not available on PDF or [Comic Book Archive] in v1 — the anchor is EPUB CFI ([ADR 0028]); a page-anchored variant is on the roadmap alongside PDF annotations.

### Word Lookup
A user-triggered lookup of selected EPUB text for its definition or lexical explanation, captured as reading history rather than as a user-authored mark. A Word Lookup belongs to the selected term and may remember the [Library Item] and reading context where it was made, but it is distinct from an [Annotation] and does not surface in the [Annotations View].

### Dictionary Pack
A locally installed dictionary dataset used to resolve [Word Lookup]s offline. A Dictionary Pack is selected by language and may be refreshed from its upstream data source, but it is not a [Source] because it has no browsable library and does not provide Library Items.

### English Gloss
An English-language definition or lexical explanation returned for a [Word Lookup], even when the looked-up term comes from a non-English book. English Glosses are the v1 display language for dictionary results; translation-quality bilingual study is a separate future concern.
...
### Emphasis
A marked **text range** in an EPUB carrying a *set* of typographic styles the user wants applied on top of the publisher's rendering: any non-empty subset of **{bold, italic, underline, strike}**. Distinct from [Highlight]: Emphasis has no colour axis and no [Note]; it mutates how the range renders rather than painting a background behind it. A range may carry a Highlight *and* an Emphasis simultaneously — the two annotations are independent rows over the same CFI range and combine freely; no combination is forbidden, no ordering matters (all four styles + a highlight is a legal state). Anchored to a **CFI range** (start + end), like Highlight. An [Annotation].

Rendering is **literal**, not a word-processor toggle: applying `bold` always sets `font-weight: bold` for the range, regardless of the publisher's baseline styling. The stored intent (`styles = {bold}`) and the on-screen result are always the same string of CSS — so a synced annotation renders identically on devices whose fonts or dark-mode CSS differ. To prevent silent no-ops when the range is entirely inside a publisher run of the same style, the range's action sheet greys out styles the baseline already provides ("already italic"); a *partial* overlap leaves the button enabled and applies literally to the whole range.

Removal is axis-symmetric: the `∅` swatch on the Highlight row removes the Highlight only; toggling all four Emphasis chips off leaves an empty set that garbage-collects on sheet dismiss (so the row's provenance survives an in-flight edit); the sheet's explicit **Delete** action removes every annotation carried by the range — both the Highlight and the Emphasis if both are present.

Two Emphasis rows auto-merge iff their `styles` sets are identical **and** their ranges are adjacent (touching or with only whitespace in the gap), chain-merged loop-until-stable — the same mechanism as [Highlight] auto-merge, with `styles` playing the role of `color` and no-note gate collapsed (Emphasis has no note). Merging is blocked when a figure sits in the gap, for the same "two separate gestures preserved" rationale. Rows with *different* `styles` sets never merge — they stay independent and the renderer unions their styles character-by-character at paint time. Same-range writes edit the existing row (mirror of `highlightOverlapsAtSamePosition`). Toggling a style off from a partial selection of a merged row *shrinks* the row's range to the un-selected side — the visible mark on the selection goes away; storage slices to match.
...
`developerModeEnabled` also gates the **[O'Reilly]** card in the "Add source" picker: the card is hidden unless dev mode is unlocked. The gate is **add-time only** — once an O'Reilly Source is configured, it behaves like any other [Web Source] and is unaffected by the flag, so toggling dev mode off never hides a configured Source or orphans its downloads; it only prevents adding a *new* one. The gate is implemented against the cross-platform `DeveloperOptionsRepository` interface (`core:domain/commonMain`) so it activates on iOS automatically once iOS grows a `DeveloperOptionsRepository` implementation and unlock surface — which are Android-only today and a separate future slice, not part of the O'Reilly work.
