...
### Web Source
A Source backed by a remotely browsable web catalogue rather than the user's persistent server library. Web Sources expose public or catalogue-owned works for browsing and file acquisition; they do not represent a user-owned server library. Shipping Web Sources are **Chitanka** and **Gutenberg**. A Web Source is distinct from a [Server Source], even though both may use HTTP. [LocalFiles] is neither: it is the device's own file origin.
