...
class TocIntegrationTest {
    ...
    @Test
    fun railSegmentsForChapter2AreItsSubchapters() = runTest {
        val pub = openTestEpub()
        val entries = pub.tableOfContents.toTocEntries()

        val segments = buildRailSegments(entries, "chapter2.xhtml")
        assertEquals("Rail should show the 3 subchapters of chapter 2", 3, segments.size)
        assertTrue("Segment 0 href should be chapter2.xhtml#s1", segments[0].href.contains("s1"))
        assertTrue("Segment 1 href should be chapter2.xhtml#s2", segments[1].href.contains("s2"))
        assertTrue("Segment 2 href should be chapter2.xhtml#s3", segments[2].href.contains("s3"))
    }

    @Test
    fun activeSegmentIsSection23WhenLocatorIsAtChapter2Section3() = runTest {
        val pub = openTestEpub()
        val entries = pub.tableOfContents.toTocEntries()
        ...
        assertEquals("Section 2.3 (index 2) should be active for chapter2.xhtml#s3", 2, activeIndex)
    }
