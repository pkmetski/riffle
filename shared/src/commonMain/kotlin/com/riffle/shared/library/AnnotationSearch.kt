...
/** One annotation match in a library search, carrying its owning book's display fields. */
 ...
 * title + cover. Returns empty for a blank query.
 */
fun searchAnnotations(
    ...
    return annotations.mapNotNull { a ->
        val item = byId[a.itemId] ?: return@mapNotNull null
...
/**
 * Filter [bookmarks] to those whose title contains [query] (case-insensitive), scoped to
 * [libraryItems], and pair each with its book's title + cover. Returns empty for a blank query.
 */
